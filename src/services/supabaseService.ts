...
export class SupabaseService {
  // 通用查询方法
  static async getAll<T>(table: string, columns: string = '*', options?: {
  ...
  // 通用获取单条记录方法
  static async getById<T>(table: string, id: number, columns: string = '*'): Promise<{ data: T | null, error: any }> {
    if (!supabaseEnabled) {
  ...
  // 通用更新方法
  static async update<T>(table: string, id: number, data: Partial<T>): Promise<{ data: T | null, error: any }> {
    if (!supabaseEnabled) {
