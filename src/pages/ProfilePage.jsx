...
const Badge = ({
  ...
  <div className="text-center">
    <div
      className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center border-2 ${borderColorClass} ${bgColorClass} ${isUnlocked ? '' : 'grayscale opacity-50'}`}
    >
  ...
  const fetchProfile = async () => {
    ...
    if (!supabaseEnabled || !user) {
      ...
      setProfile({
        ...
        single_trade_max: 10000.0,
        daily_trade_max: 50000.0,
      });
      setLoading(false);
      return;
    }
    ...
    try {
      const { data, error } = await supabase
        .from('users')
        .select(
          'username, current_balance, fund_permission, option_permission, sh_contract_permission, hk_contract_permission, single_trade_max, daily_trade_max',
        )
        .eq('id', user.id)
        .single();
