...
export const OptionTrading = () => {
  ...
  const loadOptions = async () => {
    ...
    try {
      if (!supabaseEnabled) {
        setOptions([
          { id: 1, option_code: 'OPT001', option_name: '1分钟涨跌期权', cycle: 1, yield_rate: 75, min_invest: 100 },
          { id: 2, option_code: 'OPT002', option_name: '5分钟涨跌期权', cycle: 5, yield_rate: 78, min_invest: 100 },
          { id: 3, option_code: 'OPT003', option_name: '10分钟涨跌期权', cycle: 10, yield_rate: 80, min_invest: 100 }
        ]);
      } else {
                ...
                <div className="text-sm text-gray-600">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {selectedOption.cycle && (
                      <div><span className="font-medium">周期：</span>{selectedOption.cycle}分钟</div>
                    )}
                    {selectedOption.yield_rate && (
                      <div><span className="font-medium">收益率：</span>{selectedOption.yield_rate}%</div>
                    )}
                    {selectedOption.min_invest && (
                      <div><span className="font-medium">最小投资：</span>{formatCurrency(selectedOption.min_invest)}</div>
                    )}
                    {amount > 0 && selectedOption.yield_rate && (
                      <div><span className="font-medium">预期收益：</span>{formatCurrency(amount * selectedOption.yield_rate / 100)}</div>
                    )}
                  </div>
                </div>
              ) : null;
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {orders.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-4 text-center text-gray-500">暂无交易记录</td>
                  </tr>
                ) : (
                  orders.map(order => (
